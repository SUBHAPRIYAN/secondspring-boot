...
public class FirstController {
    ...
    public ResponseEntity<First> getUserById(@PathVariable("id") Long id) {

        Optional<First> first2 = firstRepository.findById(id);

        if (first2.isPresent()) {
            First first3 = first2.get();
            return new ResponseEntity<First>(first3, HttpStatus.OK);
        }
    ...
    public ResponseEntity<HttpStatus> delteUser(@PathVariable("id") Long id) {
